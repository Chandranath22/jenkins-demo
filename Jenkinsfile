  ...
  agent any
    stages {
      ...
      stage ('Deploying SAM template') {
        steps {
          echo '******************* Deploying *************************'
          bat label: 'Sam deploying', script: 'sam deploy --stack-name calc-demo --region ap-south-1 --capabilities CAPABILITY_IAM --confirm-changeset false --s3-bucket jenkins-demo-101'
        }
      }
    }
}
